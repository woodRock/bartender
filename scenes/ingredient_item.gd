...
func _apply_resource_data():
	...
	if sprite:
		...
		if pour_particles:
			...
			pour_particles.z_index = 20
			pour_particles.local_coords = false 
...
func _process(delta):
	if dragging:
		...
		var shaker_ref = _get_shaker_ref()
		var over_shaker = shaker_ref != null
		
		# Rotation logic
		var target_tilt = deg_to_rad(90.0) if over_shaker else clamp(velocity.x * -0.015, -0.6, 0.6)
		rotation = lerp_angle(rotation, target_tilt, 0.15)
		
		_handle_pouring_visuals_and_data(shaker_ref)
...
func _handle_pouring_visuals_and_data(shaker):
	var n = ingredient_name.to_lower()
	var deg_rot = abs(rad_to_deg(rotation))
	var is_tilted = deg_rot > 50.0 or shaker != null
	...
	if is_tilted:
		...
		elif not data.is_solid: _play_loop(snd_pour)
		
		# Continuous data send to Shaker
		# (The shaker script now handles filtering this so it only fills 1/8th once)
		if shaker:
			shaker.add_ingredient(data.name, data.color, data.is_solid, data.density)
	else:
		_stop_pouring()
...
func _get_shaker_ref():
	# Detection via group/distance for high reliability
	var shaker_node = get_tree().get_first_node_in_group("shaker")
	if shaker_node:
		if global_position.distance_to(shaker_node.global_position) < 200:
			return shaker_node
	return null
...
func _check_drop():
	# Logic for items that trigger a sound specifically when "dropped" into shaker
	var shaker_ref = _get_shaker_ref()
	if shaker_ref:
		var n = ingredient_name.to_lower()
		if "ice" in n:
